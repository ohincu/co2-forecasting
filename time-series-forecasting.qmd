...
my_theme <- function(text_color) {
  theme_bw() +
    theme(text = element_text(size = 10, family = "Lato", face = "bold", color = text_color),
...
```{r}
diff_co2 <- diff(diff(diff(co2_train, differences = 1), lag = 12), lag = 12)

# Apply Ljung-Box test
ljung_box_result <- Box.test(diff_co2, lag = 12, type = "Ljung-Box")
...
Nevertheless, I will forecast the next 36 years up to 2050 to check this assumption.
